...
const METRICS = [
  { key: 'answerCorrectness', label: 'Độ chính xác câu trả lời', plain: 'Câu trả lời khớp với đáp án chuẩn đến mức nào.', direction: 'Cao hơn tốt hơn', formula: 'RAGAS AnswerCorrectness dùng judge và semantic similarity với đáp án chuẩn.' },
  { key: 'answerRelevance', label: 'Mức liên quan của câu trả lời', plain: 'Câu trả lời có đi đúng trọng tâm câu hỏi hay không.', direction: 'Cao hơn tốt hơn', formula: 'RAGAS AnswerRelevancy đánh giá câu trả lời theo câu hỏi, độc lập với ground truth.' },
  { key: 'faithfulness', label: 'Bám sát nguồn', plain: 'Câu trả lời RAG có được hỗ trợ bởi tài liệu đã truy xuất hay không.', direction: 'Cao hơn tốt hơn', formula: 'RAGAS Faithfulness kiểm tra các phát biểu trong câu trả lời với retrieved contexts.', ragOnly: true },
  { key: 'contextPrecision', label: 'Độ chính xác nguồn tìm được', plain: 'Các đoạn RAG lấy về được xếp hạng hữu ích đến mức nào.', direction: 'Cao hơn tốt hơn', formula: 'RAGAS ContextPrecision dùng reference answer để đánh giá thứ hạng contexts.', ragOnly: true },
  { key: 'contextRecall', label: 'Độ bao phủ nguồn', plain: 'RAG đã lấy đủ thông tin cần thiết để trả lời hay chưa.', direction: 'Cao hơn tốt hơn', formula: 'RAGAS ContextRecall kiểm tra phần thông tin trong reference được contexts bao phủ.', ragOnly: true },
]
      ...
      {loading ? <Panel className="mt-4 flex min-h-40 items-center justify-center gap-2 p-5 text-sm font-semibold text-slate-600"><span className="size-4 animate-spin rounded-full border-2 border-primary border-r-transparent" />Đang tạo báo cáo…</Panel> : comparison ? (
        <article className="research-report mt-4 space-y-6" aria-label="Báo cáo thực nghiệm RAG và Fine-tuned">
          ...
          <section aria-labelledby="benchmark-heading">
            ...
            <div className="mt-3 flex items-start gap-2 rounded-xl bg-teal-50 p-3 text-sm leading-6 text-teal-900"><Info className="mt-0.5 shrink-0" size={17} /><p><strong>Chuẩn đánh giá:</strong> Official RAGAS 0.4.3 với judge gpt-4o-mini và evaluator embedding text-embedding-3-small. Chỉ các run được đánh giá thành công toàn bộ mới được so sánh.</p></div>
          </section>
...
function ReportTitle({ comparison, onCsv, onPrint }) {
  ...
  return <header className="os-panel overflow-hidden p-6 sm:p-8">
    ...
    <dl className="mt-6 grid gap-4 border-t border-slate-200 pt-5 text-sm sm:grid-cols-2 lg:grid-cols-4"><Meta label="Số câu hỏi" value={dataset.questionCount ?? comparison.perQuestion?.length ?? 0} /><Meta label="Số tài liệu" value={dataset.documentCount ?? '—'} /><Meta label="Chuẩn metric" value={comparison.label ?? 'Official RAGAS'} /><Meta label="Evaluator" value={comparison.evaluatorModel ?? 'gpt-4o-mini'} /></dl>
