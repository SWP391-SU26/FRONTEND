        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1080px] border-collapse text-left text-sm">
            <thead className="bg-slate-50 text-xs font-black uppercase tracking-wide text-slate-500">
              <tr><th className="px-4 py-3">Plan</th><th className="px-4 py-3">Price / duration</th><th className="px-4 py-3">Per file</th><th className="px-4 py-3">Documents</th><th className="px-4 py-3">Storage</th><th className="px-4 py-3">Workspaces</th><th className="px-4 py-3">Status</th><th className="w-28 px-3 py-3"><span className="sr-only">Actions</span></th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? <tr><td className="px-4 py-16 text-center font-bold text-slate-500" colSpan="8"><Loader2 className="mr-2 inline animate-spin" size={17} />Loading plans...</td></tr> : null}
              {!loading && !plans.length ? <tr><td className="px-4 py-16 text-center font-bold text-slate-500" colSpan="8">No plans configured.</td></tr> : null}
              {!loading && plans.map((plan) => (
                <tr className="hover:bg-slate-50" key={plan.planId}>
                  ...
                  <td className="px-4 py-4 font-bold text-slate-700">{plan.maxDocuments}</td>
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <form className="h-full w-full max-w-2xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8" onSubmit={onSave}>
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          ...
          <Field label="Maximum documents"><input className="control" min="1" onChange={(event) => onUpdate('maxDocuments', event.target.value)} required type="number" value={form.maxDocuments} /></Field>
